...
from config import PUBLIC_APP_BASE_URL


logger = logging.getLogger("nightshift.estimate")


def _job_url(submission_id) -> str:
    """Absolute link to the job's status page, used by the DRAFT banner so an
    estimator can jump straight to Rerun with Revisions."""
...
<body>
    ...
    <div class="review-banner">
        <div class="rb-title">DRAFT — PLEASE REVIEW BEFORE SUBMITTING YOUR BID</div>
        <div class="rb-body">This estimate was generated by KnightShiftAI and
        we recommend a manual review before it goes out for bid. We do our
        best, but sometimes items are missed or pages are dropped. Please take
        a moment to:</div>
        <ul>
            <li>Review the results against your scope.</li>
            <li>Review any open RFIs — if you can answer them, use
...
def generate_estimate_pdf(submission, organization, result: dict, out_dir: str,
    ...
    html_str = _HTML_TEMPLATE.render(
        ...
        job_url=_job_url(submission.id),
